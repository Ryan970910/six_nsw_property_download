...
from .db import DEFAULT_PROPERTY_TABLE, PostgresConfig, check_dat_rows_against_db, upload_dat_rows_with_skip_report
...
def check_weekly_dat_new_records(
    ...
    *,
    ...
    logger.info("weekly_dat_check_pipeline_start weeks=%s target_table=%s work_dir=%s", [w.isoformat() for w in weeks], target_table, work_dir)
    ...
    for week in weeks:
        logger.info("weekly_dat_check_week_start week=%s", week.isoformat())
        ...
        check_result = check_dat_rows_against_db(db_config, uploadable_rows, target_table=target_table)

        skipped_rows = duplicate_rows + invalid_rows + check_result.existing_rows
        ...
        total_uploadable += check_result.copied_rows
        total_new += check_result.new_rows
        total_existing += len(check_result.existing_rows)
        week_info = {
            "week": week.isoformat(),
            "files": parsed.files,
            ...
            "uploadable_rows": check_result.copied_rows,
            "existing_rows": len(check_result.existing_rows),
            "new_rows": check_result.new_rows,
            "skipped_rows": len(skipped_rows),
        }
        week_results.append(week_info)
        logger.info("weekly_dat_check_week_complete %s", week_info)

    report_path = None
    if all_skipped:
        dates_part = "_".join(week.strftime("%Y%m%d") for week in weeks)
        report_path = report_dir / f"weekly_dat_dry_run_skipped_{dates_part}.xlsx"
        write_skipped_rows_xlsx(all_skipped, report_path)
        logger.info("weekly_dat_check_skipped_report_written path=%s rows=%s", report_path, len(all_skipped))

    logger.info(
        "weekly_dat_check_pipeline_complete weeks=%s files=%s parsed_rows=%s uploadable_rows=%s new_rows=%s existing_rows=%s skipped_rows=%s",
    ...
    return WeeklyCheckResult(
        weeks=weeks,
        downloaded_files=total_files,
        parsed_rows=total_parsed,
        uploadable_rows=total_uploadable,
        new_rows=total_new,
